...
class WeComClient:
    ...
    async def send_text(self, content: str) -> bool:
        ...
        # 企业微信 text.content 的上限为 2048 个字节；保守截断以避免 API 拒绝。
        ...
        for attempt in range(3):
            try:
                ...
                if result.get("errcode") == 0:
                    logger.debug("企业微信应用消息发送成功")
                    return True
                if result.get("errcode") in self.TOKEN_RETRY_CODES and attempt == 0:
                    await self.get_access_token(force_refresh=True)
                    continue
                logger.error("企业微信应用消息发送失败：%s %s", result.get("errcode"), result.get("errmsg"))
                return False
            except (httpx.HTTPError, RuntimeError, ValueError) as exc:
                if attempt == 2:
                    logger.error("企业微信应用消息发送异常：%s", exc)
                    return False
...
async def send_wecom_message(text: str) -> bool:
    if not wecom_client:
        return False
...
async def emby_webhook_handler(request: web.Request) -> web.Response:
    try:
        ...
        wecom_sent = await send_wecom_message(notification_text) if wecom_client else False

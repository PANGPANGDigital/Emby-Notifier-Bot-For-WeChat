...
class Config:
    ...
    # 飞书群自定义机器人。FEISHU_WEBHOOK_URL 是唯一必填参数。
    FEISHU_WEBHOOK_URL = os.getenv("FEISHU_WEBHOOK_URL", "")
    FEISHU_PROXY = os.getenv("FEISHU_PROXY")
    FEISHU_WEBHOOK_URLS = [u.strip() for u in FEISHU_WEBHOOK_URL.split(",") if u.strip()]
    ...
    @classmethod
    def feishu_configured(cls) -> bool:
        return bool(cls.FEISHU_WEBHOOK_URLS)
...
if not Config.telegram_configured() and not Config.wecom_configured() and not Config.feishu_configured():
    raise RuntimeError(
        "至少配置一个通知通道：Telegram（TELEGRAM_BOT_TOKEN、TELEGRAM_CHAT_ID）或"
        "企业微信（WECOM_CORP_ID、WECOM_AGENT_ID、WECOM_CORP_SECRET）或"
...
async def send_wecom_message(text: str, image_url: Optional[str] = None) -> bool:
    ...
    if image_url:
        ...
        rest_lines = lines[1:] if lines and lines[0].strip() == title.strip() else lines
    ...
    return await wecom_client.send_text(f"{text}\n\n{get_emoji('time')} 时间: {timestamp}")
...
FEISHU_POST_MAX_DESC_BYTES = 4096
FEISHU_TITLE_MAX_BYTES = 128
FEISHU_IMAGE_UPLOAD_URL = "https://open.feishu.cn/open-apis/im/v1/images"
FEISHU_MSG_API_TIMEOUT = 15.0
...
def _build_feishu_post_content(text: str) -> Tuple[str, List]:
    lines = text.split("\n")
    non_empty = [l for l in lines if l.strip()]
    if non_empty:
        first = non_empty[0].strip()
        title = _truncate_to_byte_limit(first, FEISHU_TITLE_MAX_BYTES)
        if title == first:
    ...
    desc = _truncate_to_byte_limit(raw_desc, FEISHU_POST_MAX_DESC_BYTES)
    content = _parse_feishu_lines(desc)
    return title, content


def _parse_feishu_lines(text: str) -> List:
    result: List = []
    for raw_line in text.split("\n"):
        paragraph: List = []
        parts = re.split(r'(\[.*?\]\(.*?\)|https?://\S+)', raw_line)
        for part in parts:
            if not part:
                continue
            if re.match(r'\[.*?\]\(.*?\)', part):
                m = re.match(r'\[(.*?)\]\((.*?)\)', part)
                if m:
                    paragraph.append({"tag": "a", "text": m.group(1), "href": m.group(2)})
                    continue
            if part.startswith("http://") or part.startswith("https://"):
                paragraph.append({"tag": "a", "text": part, "href": part})
                continue
...
class FeishuClient:
    def __init__(self) -> None:
        proxy = NetworkUtils._normalize_proxy(Config.FEISHU_PROXY)
        self.client = httpx.AsyncClient(
            timeout=httpx.Timeout(FEISHU_MSG_API_TIMEOUT),
            headers={"User-Agent": f"EmbyNotifier/Feishu/{APP_VERSION}"},
            proxies=proxy or {},
        )
    ...
    async def _post(self, url: str, payload: dict) -> bool:
        try:
            ...
            if body.get("code") == 0:
                return True
            logger.error("飞书消息发送失败：%s %s", body.get("code"), body.get("msg"))
            return False
        except httpx.HTTPError as exc:
            logger.error("飞书消息发送异常：%s", exc)
            return False
    ...
    async def send_post(self, title: str, content: List) -> bool:
        ...
        tasks = [self._post(url, payload) for url in Config.FEISHU_WEBHOOK_URLS]
        results = await asyncio.gather(*tasks, return_exceptions=True)
        return any(r is True for r in results)

    async def send_image(self, image_bytes: bytes, image_key: str = "") -> bool:
        payload = {
            "msg_type": "image",
        ...
        if not image_key:
            try:
                ...
                logger.warning("飞书封面上传失败，跳过图片消息：%s", exc)
                return False
            if not image_key:
                return False
        payload["content"]["image_key"] = image_key
        tasks = [self._post(url, payload) for url in Config.FEISHU_WEBHOOK_URLS]
        results = await asyncio.gather(*tasks, return_exceptions=True)
        return any(r is True for r in results)

    async def _upload_image(self, image_bytes: bytes) -> str:
        files = {"image": ("cover.jpg", BytesIO(image_bytes), "image/jpeg")}
        ...
        raise RuntimeError(f"上传失败：{body.get('code')} {body.get('msg')}")


feishu_client = FeishuClient() if Config.feishu_configured() else None


async def send_feishu_message(text: str, image_url: Optional[str] = None) -> bool:
    if not feishu_client:
        return False

    title, content = _build_feishu_post_content(text)
    post_sent = await feishu_client.send_post(title, content)
    ...
    if image_url and image_url.strip():
        _img_downloader = _ImageDownloader(Config.FEISHU_PROXY)

        try:
            image_bytes = await _img_downloader.get(image_url.strip())
            if image_bytes:
                await feishu_client.send_image(image_bytes)
        except Exception as exc:
            logger.warning("飞书封面上传失败：%s", exc)
        finally:
            await _img_downloader.close()
...
def get_emby_image_url(item: Dict, for_wecom: bool = False) -> Optional[str]:
    if not EMBY_SERVER_URL:
        return None
    ...
    cache_key = f"img_{item_id}_{for_wecom}"
    cached = image_cache.get(cache_key)
    if cached is not None:
        return cached

    image_types = ["Backdrop", "Fanart", "Primary"]
    max_width = 1600 if for_wecom else 1200
    max_width_primary = 1200 if for_wecom else 800
    item_type = item.get("Type", "")

    def _build_url(path: str, tag: str, width: int) -> str:
        params = [f"tag={tag}", "quality=90", f"maxWidth={width}"]
        if EMBY_API_KEY:
            params.append(f"api_key={EMBY_API_KEY}")
        return f"{path}?{'&'.join(params)}"

    if item_type == "Episode":
        episode_backdrop_tags = item.get("BackdropImageTags", [])
        if episode_backdrop_tags:
            url = _build_url(
                f"{EMBY_SERVER_URL.rstrip('/')}/Items/{item_id}/Images/Backdrop",
                episode_backdrop_tags[0],
                max_width,
            )
        ...
        if series_id and "Primary" in series_image_tags:
            url = _build_url(
                f"{EMBY_SERVER_URL.rstrip('/')}/Items/{series_id}/Images/Primary",
                series_image_tags["Primary"],
                max_width_primary,
            )
            image_cache[cache_key] = url
            return url
    ...
    if item_type in ["Music", "Audio"]:
        ...
        if album_id and "Primary" in album_image_tags:
            url = _build_url(
                f"{EMBY_SERVER_URL.rstrip('/')}/Items/{album_id}/Images/Primary",
                album_image_tags["Primary"],
                max_width_primary,
            )
            image_cache[cache_key] = url
            return url
    ...
    url = _build_url(
        f"{EMBY_SERVER_URL.rstrip('/')}/Items/{item_id}/Images/Primary",
        "",
        max_width_primary,
    )
    image_cache[cache_key] = url
    ...
    try:
        ...
        if event_type == "user.authenticated":
            ...
            if event_type in ["playback.stop", "playback.pause"]:
                ...
                notification_text = "\n".join(caption)
                image_url = get_emby_image_url(item, for_wecom=True) if ENABLE_COVERS else None
        ...
        feishu_sent = await send_feishu_message(notification_text, image_url) if feishu_client else False
        ...
        delivered = wecom_sent or feishu_sent or any(msg_id is not None for msg_id in send_results.values())
        if delivered:
...
async def main():
    ...
    startup_info = (
        ...
        f"{get_emoji('info')} 飞书群机器人: {'已启用（' + str(len(Config.FEISHU_WEBHOOK_URLS)) + ' 个webhook）' if feishu_client else '未启用'}\n"
    ...
    if feishu_client:
        await send_feishu_message(startup_info)
